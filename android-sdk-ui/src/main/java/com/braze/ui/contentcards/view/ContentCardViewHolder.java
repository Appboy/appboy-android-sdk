...
public class ContentCardViewHolder extends RecyclerView.ViewHolder {
  ...
  public ContentCardViewHolder(final View view, boolean showUnreadIndicator) {
    ...
    mUnreadBar = view.findViewById(R.id.com_braze_content_cards_unread_bar);
    if (mUnreadBar != null) {
      if (showUnreadIndicator) {
        mUnreadBar.setVisibility(View.VISIBLE);
        // Round the edges at the bottom
        mUnreadBar.setBackground(view.getContext().getResources().getDrawable(R.drawable.com_braze_content_cards_unread_bar_background));
      } else {
        mUnreadBar.setVisibility(View.GONE);
      }
    }

    mPinnedIcon = view.findViewById(R.id.com_braze_content_cards_pinned_icon);
    mActionHint = view.findViewById(R.id.com_braze_content_cards_action_hint);
  }
