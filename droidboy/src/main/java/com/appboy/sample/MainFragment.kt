...
class MainFragment : Fragment() {
  ...
  override fun onCreateView(layoutInflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
    ...
    customEventsAndPurchasesArrayAdapter = ArrayAdapter(requireContext(), android.R.layout.simple_list_item_1, getLastSeenCustomEventsAndPurchasesFromLocalStorage())
    customEventOrPurchaseTextView.setAdapter<ArrayAdapter<String?>>(customEventsAndPurchasesArrayAdapter)
    ...
    contentView.setOnButtonClickWithEditableText(R.id.com_appboy_sample_set_sdk_auth_signature_button, R.id.com_appboy_sample_set_sdk_auth_signature_edit_text) { _, signature ->
      if (!StringUtils.isNullOrBlank(signature)) {
        Braze.getInstance(requireContext()).setSdkAuthenticationSignature(signature)
        Toast.makeText(requireContext(), "Set signature to: $signature", Toast.LENGTH_SHORT).show()
      } else {
  ...
  private fun handleAliasClick() {
    ...
    Braze.getInstance(requireContext()).getCurrentUser(object : IValueCallback<BrazeUser?> {
      override fun onSuccess(value: BrazeUser) {
        ...
        Toast.makeText(requireContext(), "Added alias " + alias + " with label "
